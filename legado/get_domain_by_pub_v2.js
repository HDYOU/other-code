...
function test_url_func(test_url_list, test_host_list) {
    ...
    for (let resii = 0; resii < test_url_list.length; resii++) {
        ...
        `
        thread_list.push(js_code);
    }
    
    rep_all=multi_thread_js_code(thread_list)
    for (let resii = 0; resii < rep_all.length; resii++) {
       let __time=rep_all[resii];
       let tmp_host = test_host_list[resii];
...
function handle_pub_html_list(html_dict) {
    ...
    function get_url_list_by_fabu(fby_url_list) {
        ...
        for (let resii = 0; resii < fby_url_list.length; resii++) {
            ...
            let js_code=`
                try {
                    resq_test_url = '${fby_url_list[resii]}';
                    let s_time = new Date().getTime();
                    conn = java.ajax(resq_test_url);
                    //if (conn) html += conn
            ...
            fai_thread_list.push(js_code);
        }
        
        rep_all=multi_thread_js_code(fai_thread_list)
        for (let resii = 0; resii < rep_all.length; resii++) {
          __html=rep_all[resii] || "";
          let resq_test_url = fby_url_list[resii]
          html_dict[resq_test_url] = __html
        }
