...
function test_and_get_best_host(
    ...
    for (let resii = 0; resii < test_url_list.length; resii++) {
        ...
        let thread = new Promise(function (resolve, reject) {
            let __time = bed_time;
            try {
                ...
                __time = new Date().getTime() - s_time;
...
function handle_thread(thread_list) {
    let _flag=false
    Promise.all(thread_list)
     .then((values) => _flag=true)
     .catch((error) => _flag=true)
            ...
            let thread = new Promise(function (resolve, reject) {
               let __time = bed_time;
                try {
                    ...
                    __time = e_time - s_time;
                    java.log("time: " + __time + "\t host: " + resq_test_url)
                } catch (e) {
                    tt = 1
                    java.log(`pub ul error: ${resq_test_url} \n ${e.message.substring(0,100)}`)
                }
                resolve(__time)
            });
            //thread.start();
