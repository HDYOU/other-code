...
function getHost() {
    var source = this.source,
        ...
        var cache_host_key="croxy host"+source.key;
        ...
        for (var i = 0; i < MAX_RETRIES; i++) {
            try {
                ...
                if (locationHeader) {
                    ...
                    // 保存到缓存
                    cache.put(cache_ck_key, cook);
                    cache.put(cache_time_key, time, CACHE_TIMEOUT_SEC);
                    cache.put(cache_host_key, host);
                    return host;
                }
            } catch (e) {
                java.log(
...
function creatRequest(urlPath, params, method="GET", body, charset) {
    ...
    const { source, java, cache } = ctx;
        ...
        var cache_lock_key="croxy lock"+source.key;

    // 检查缓存是否有效
    function isValidCache() {
        var time = cache.get(cache_time_key);
        if (!time) return false;

        var timeDiff = Date.now() - parseInt(time, 10);
        return cache.get(cache_host_key) && timeDiff <= CACHE_TIMEOUT_SEC * 1000;
    }
    ...
    function fu() {
        let headers = (() => {
            ...
            //let ref = source.key + "/";
            let cookie = cache.get(cache_ck_key);
    ...
    if (!cache.get(cache_lock_key)) {
        try {
            cache.put(cache_lock_key, 1, LOCK_TIMEOUT_SEC);
            ctx.getHost();
        } catch (error) {
        ...
        while (cache.get(cache_lock_key) && waitTime < WAIT_MAX_MS) {
            Packages.java.lang.Thread.sleep(INTERVAL);
            waitTime += INTERVAL;

            if (isValidCache()) {
                return String(cache.get(cache_host_key));
            }
        }

        if (waitTime/1000 >= WAIT_MAX_MS && cache.get(cache_lock_key)) {
            java.log("等待缓存更新超时");
        }
    }
...
function clearCacheWith(cache) {
	...
	cache.delete(cache_ck_key);
	cache.delete(cache_time_key);
	cache.delete(cache_host_key);
	cache.delete(cache_lock_key);
	java.toast("缓存已清空")
}